...
void eps_set_pin_value( int pin, int value) {
...
void eps_send_action( uint8_t dest, uint8_t action )
{
    ...
    Wire.eps_WRITE( BOARD_ID );
    Wire.eps_WRITE( action );
...
void eps_send_version( int dest )
{
    Wire.beginTransmission( dest );
    Wire.eps_WRITE( BOARD_ID );
    Wire.eps_WRITE( EPS_VERSION );
    Wire.eps_WRITE( VERSION );
    Wire.endTransmission();
}
...
void eps_send_board_update(uint8_t dest) 
{
    if ( ! boards[dest].pin_update_queue.isEmpty() ) 
    {
        ...
        while ( !boards[dest].pin_update_queue.isEmpty() && count < BUFFER_LENGTH-2) 
        {
            ...
            Wire.eps_WRITE( up.type );
            Wire.eps_WRITE( up.pin );
            if ( up.type == EPS_SET ) {
                value = boards[dest].read_bpin(up.pin);
                Wire.eps_WRITE( (byte) ( ( value & 0xFF00)>>8 ) );
                Wire.eps_WRITE( (byte) ( value & 0x00FF ) );
                count = count+2+2;
            }
            else if ( up.type == EPS_SETUP )
            {
                Wire.eps_WRITE( boards[dest].read_bpin_type(up.pin) );
                count = count+2+1;
            }
        }
        Wire.endTransmission();
